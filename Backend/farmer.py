...
def _listing(row):
    return {
        ...
        "unit": row[4],
        "expected_harvest_date": row[6].isoformat() if row[6] else None,
        "quality_details": row[7],
        "expected_price": float(row[8]) if row[8] is not None else None,
        "preferred_market": row[9],
        "image_urls": row[10] or [],
        "status": row[11],
        "buyer_interest_count": row[12],
