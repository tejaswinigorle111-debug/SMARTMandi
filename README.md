...
The API requests live market prices from data.gov.in when `DATA_GOV_API_KEY`, `DATA_GOV_RESOURCE_ID`, and `DATA_GOV_PRICE_UNIT` are configured. If the live service is unavailable or not configured, the API reports that market data is unavailable rather than presenting unverified prices.
